...
/* Converting development code to production code. ? */

// Reference counting should be at an higher level than the string library.
// len allows max 32 Gb. Compromise between strings length and memory footprint of this param depending on the number of strings.

// Can refcount overflow? I can set a leak (no incremente or decrement a string that reach max)
// If I want to always have a string allocated (static object), I set its refcount to its max.
// This way I can avoid dereferencing some defined strings.
